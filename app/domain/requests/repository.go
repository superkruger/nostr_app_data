...
func (r *repository) add(ctx context.Context, request Request) error {
	return xray.Capture(ctx, "DB - add request", func(ctx1 context.Context) error {
		_, err := r.c.UpdateOne(ctx1, bson.M{"id": request.ID}, bson.M{"$set": request}, &options.UpdateOptions{Upsert: jsii.Bool(true)})
		return err
	})
}

func (r *repository) remove(ctx context.Context, id string) error {
	return xray.Capture(ctx, "DB - remove request", func(ctx1 context.Context) error {
